...
export default function AnalyticsPage() {
  ...
  // State for selected member detailed view
  ...
  // Compute Agency Metrics based on tasks
  ...
  const clientStats = clients
    ...
    .map(client => {
      ...
      const monthlyBudget = client.monthlyPointBudget || 5000;
  ...
  return (
    <div className="space-y-6 animate-fadeIn text-neutral-900 pb-12">
      ...
      <div className="bg-white rounded-2xl border border-neutral-200 p-5 space-y-4 shadow-xs">
        <div>
          <h2 className="text-sm font-bold text-neutral-800 flex items-center gap-2">
            <Briefcase className="w-4 h-4 text-emerald-600" /> Client Point Utilization & Gross Billing Review
          </h2>
          <p className="text-[10px] text-neutral-400">Track utilized points against allocated monthly quotas for active client accounts.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pt-2">
          {clientStats.map(stat => {
            ...
            return (
              <div key={stat.id} className="p-4 rounded-xl border border-neutral-100 bg-neutral-50/50 space-y-3 flex flex-col justify-between">
                <div>
                  <div className="flex items-center justify-between">
                    <span 
                      className="px-2 py-0.5 rounded text-[10px] font-bold"
                      style={{ backgroundColor: `${stat.clientColor}15`, color: stat.clientColor }}
                    >
                      {stat.name}
                    </span>
                    <span className="font-mono font-bold text-xs text-neutral-800">
                      {stat.usedPoints} / {stat.monthlyBudget} pts
                    </span>
                  </div>
                  
                  {/* Progress Bar */}
                  <div className="w-full bg-neutral-200 rounded-full h-2.5 mt-3 overflow-hidden">
                    <div 
                ...
                <div className="flex items-center justify-between text-[10px] font-semibold text-neutral-400 pt-1 font-mono">
                  <span>Gross Billing: <strong className="text-neutral-700">{formatRupiah(stat.billingValue)}</strong></span>
                  <span className="flex items-center gap-1 font-bold">
                    {isDanger ? (
                      <span className="text-red-600 flex items-center gap-0.5"><AlertTriangle className="w-3 h-3" /> Critical ({stat.rawPct.toFixed(0)}%)</span>
                    ) : isWarning ? (
                      <span className="text-amber-600">Warning ({stat.rawPct.toFixed(0)}%)</span>
                    ) : (
                      <span className="text-emerald-700 flex items-center gap-0.5"><CheckCircle2 className="w-3 h-3" /> Optimal ({stat.rawPct.toFixed(0)}%)</span>
                    )}
                  </span>
                </div>
              </div>
          ...
          {clientStats.length === 0 && (
            <div className="col-span-full py-12 text-center text-neutral-400 italic font-mono text-xs">
              No client budgets configured in this workspace.
            </div>
          )}
        </div>
      </div>
